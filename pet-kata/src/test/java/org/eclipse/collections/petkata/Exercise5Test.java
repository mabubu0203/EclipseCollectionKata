...
public class Exercise5Test extends PetDomainForKata
{
    ...
    {
        Pet oldestPet = this.people.flatCollect(person -> person.getPets()).maxBy(pet -> pet.getAge());
...
//        MutableIntSet petAges = IntSets.mutable.with(5);
        MutableIntSet petAges = this.people.flatCollect(Person::getPets).collectInt(Pet::getAge).toSet().with(5);
...
//        }
        ...
        MutableSet petAges = people
                .flatCollect(person -> person.getPets())
                .collect(Pet::getAge).toSet();
